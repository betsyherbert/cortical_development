...
from src.model.config import ANATOMICAL_GRID_SIZE, CELL_TYPES, LAYERS
...
def test_preset_spatial_parameters_in_um(preset_name):
    ...
    for cell_type in CELL_TYPES:
        ...
        assert (
            10.0 <= width <= 500.0
        ), f"thalamic_width {cell_type} = {width} should be in μm range [10, 500]"
    ...
    for cell_type in CELL_TYPES:
        ...
        assert (
            10.0 <= width <= 500.0
        ), f"outgoing_width {cell_type} = {width} should be in μm range [10, 500]"
