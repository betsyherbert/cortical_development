    ...
    for idx, k in enumerate(k_values):
        ...
        for i in range(total_pops):
            for j in range(total_pops):
                w_tilde = network.A[i, j] * exp_factors[i, j]
                if i == j:
                    J[i, j] = -1.0 / network.tau[i] + (analyzer.g_eff[i] * w_tilde) / network.tau[i]
                else:
                    J[i, j] = (analyzer.g_eff[i] * w_tilde) / network.tau[i]
...
def compute_gain_spectra_single_stage(
    stage_name: str, parameter_keys: list[str], n_processes: int = None
) -> dict:
...
def compute_gain_spectra_all_stages(
    parameter_keys: list[str], stages: list[str] = None, n_processes: int = None
) -> dict:
    ...
    for stage_name in stages:
        stage_results = compute_gain_spectra_single_stage(stage_name, parameter_keys, n_processes)
