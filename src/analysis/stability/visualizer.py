...
All visualization constants are now in config.py.
This module contains only the StabilityVisualizer class.
"""
...
# Plotting constants (layout/decoration only; line/marker sizes in config)
LINE_COLOR = "black"
...
class StabilityVisualizer:
    ...
    def _create_regime_colormap(self):
        ...
        self.regime_norm = BoundaryNorm(
            boundaries=np.arange(len(REGIME_LABELS) + 1) - 0.5, ncolors=len(REGIME_LABELS)
        )
    ...
    def _create_column_grids(self, column_data: dict, coords: list[tuple]) -> dict:
        ...
        # Regime grid
        grids["regimes"] = self._create_grid(column_data["regimes"], coords, self.column_patch_size)
    ...
    def _plot_regime_heatmap(self, ax, data):
        ...
        im = ax.imshow(
            regime_data,
            cmap=self.regime_cmap,
            norm=self.regime_norm,
            origin="lower",
            aspect="equal",
        )
        self._configure_axis(ax)
    ...
    def _plot_thalamic_input(self, ax, thalamic_input: np.ndarray):
        ...
        im = ax.imshow(thalamic_input, cmap="gray", vmin=0, vmax=1, origin="lower", aspect="equal")
        self._configure_axis(ax)
        return im
    ...
    def _add_column_wise_colorbars(self, fig, axes, ims):
        ...
        if ims["diff"]:
            ...
            self._add_colorbar(
                ...
                ims["diff"],
                positions["diff"],
                r"$\Delta\lambda_{\mathrm{max}}$",
                [-vmax_diff, 0, vmax_diff],
            )
        ...
        if ims["thalamic"]:
            self._add_colorbar(
                fig, ims["thalamic"], positions["thalamic"], "Thalamic Input", [0, 0.5, 1]
            )
    ...
    def create_layer_wise_figure(self, results: dict, regime: str, snapshot_idx: int):
        ...
        for age_idx, stage in enumerate(DEVELOPMENTAL_STAGES):
            if (
                ...
                or regime not in results[stage]
                or snapshot_idx not in results[stage][regime]
            ):
                continue
            ...
            for layer_idx, layer in enumerate(LAYERS):
                ims["lambda"] = self._plot_heatmap(
                    axes[layer_idx, base_col + 0], grids[layer]["full"]
                )
                ims["lambda"] = self._plot_heatmap(
                    axes[layer_idx, base_col + 1], grids[layer]["e_only"]
                )
                ims["diff"] = self._plot_heatmap(
                    axes[layer_idx, base_col + 2],
                    grids[layer]["full"] - grids[layer]["e_only"],
                    center_zero=True,
                )
                ims["regime"] = self._plot_regime_heatmap(
    ...
    def create_column_wise_figure(self, results: dict, regime: str, snapshot_idx: int):
        ...
        for col_idx, stage in enumerate(DEVELOPMENTAL_STAGES):
            ...
            stage_data = results[stage][regime][snapshot_idx]
            grids = self._create_column_grids(
                stage_data["column_wise"], stage_data["column_coords"]
            )

            ims["lambda"] = self._plot_heatmap(axes[0, col_idx], grids["full"])
            ims["lambda"] = self._plot_heatmap(axes[1, col_idx], grids["e_only"])
            ims["diff"] = self._plot_heatmap(
                axes[2, col_idx], grids["full"] - grids["e_only"], center_zero=True
            )
            ims["regime"] = self._plot_regime_heatmap(axes[3, col_idx], grids)
            ims["thalamic"] = self._plot_thalamic_input(
        ...
        for condition in REGIMES:
            ...
            for analysis_type in ["layer_wise", "column_wise"]:
                ...
                for stage in DEVELOPMENTAL_STAGES:
                    ...
                    for snapshot_idx in results[stage][condition]:
    ...
    def _plot_regime_percentages(
        self, ax, percentages: dict, condition: str, analysis_type: str, title: str
    ):
        ...
        for regime in reversed(REGIME_LABELS):
            values = [
                percentages[condition][analysis_type][stage].get(regime, 0)
                for stage in DEVELOPMENTAL_STAGES
            ]
            ax.bar(
                x_positions,
                ...
                label=regime.replace("\n", " "),
            )
            bottom += values

        # Configure axis
        ax.set_title(title, fontsize=FONT_SIZES["ylabel"], fontweight="bold")
        ax.set_ylim(0, 100)
    ...
    def create_regime_percentage_plots(self, results: dict):
        ...
        # 2×2 grid of square bar charts
        fig, axes = plt.subplots(2, 2, figsize=compute_subplot_figsize(2, 2))
        fig.suptitle(
            "Stability Regime Ratios", fontsize=FONT_SIZES["title"], fontweight="bold"
        )
        ...
        for row, col, condition, analysis_type, title in plot_configs:
            ...
            if col == 0:
                ax.set_ylabel("Cortical area (%)", fontsize=FONT_SIZES["ylabel"])
        ...
        for condition in REGIMES:
            for stage in DEVELOPMENTAL_STAGES:
                ...
                for snapshot_idx in results[stage][condition]:
                    ...
                    patch_effectiveness = [
                        e_only - full
                        for full, e_only in zip(lambda_full, lambda_e_only, strict=False)
                    ]
    ...
    def _extract_effectiveness_data(
        self, effectiveness: dict, condition: str
    ...
    def create_inhibition_effectiveness_plot(self, results: dict):
        ...
        # Single square subplot
        fig, ax = plt.subplots(1, 1, figsize=compute_subplot_figsize(1, 1))
    ...
    def create_layer_effectiveness_plot(self, results: dict):
        ...
        for layer in LAYERS:
            ...
            means, sems = np.array(means), np.array(sems)
            color = LAYER_COLORS[layer]  # Use internal key for color lookup
    ...
    def _calculate_layer_effectiveness(self, results: dict) -> dict:
        ...
        for layer in LAYERS:
            for stage in DEVELOPMENTAL_STAGES:
                ...
                for snapshot_idx in results[stage]["driven"]:
    ...
    def _add_reference_lines(self, ax, lim_min: float, lim_max: float):
        ...
        ax.axhline(
            ...
            color=LINE_COLOR,
            linestyle="-",
            alpha=REFERENCE_LINE_ALPHA_THICK,
    ...
    def _configure_phase_subplot(
        self, ax, stage: str, lim_min: float, lim_max: float, show_legend: bool = False
    ):
        """Configure individual phase diagram subplot."""
        ax.set_title(stage, fontsize=FONT_SIZES["ylabel"], fontweight="bold")
        ax.set_xlabel(r"$\lambda_{\mathrm{full}}$", fontsize=FONT_SIZES["ylabel"])
        ax.set_ylabel(r"$\lambda_{\mathrm{E\text{-}only}}$", fontsize=FONT_SIZES["ylabel"])
        ...
        if show_legend:
            ax.legend(fontsize=FONT_SIZES["colorbar"], frameon=True, loc="upper left")
    ...
    def create_stability_phase_diagrams(self, results: dict):
        ...
        # 1 row × 4 columns of square phase plots
        fig, axes = plt.subplots(1, 4, figsize=compute_subplot_figsize(1, 4))
        fig.suptitle(
            "Stability Phase Diagrams", fontsize=FONT_SIZES["title"], fontweight="bold"
        )
        ...
        for i, stage in enumerate(DEVELOPMENTAL_STAGES):
            ...
            if stage in phase_data:
                for condition in REGIMES:
                    if phase_data[stage][condition]["full"]:
                        ax.scatter(
                            phase_data[stage][condition]["full"],
            ...
            if i == 0:
                ax.set_ylabel(
                    r"$\lambda_{\mathrm{E\text{-}only}}$", fontsize=FONT_SIZES["ylabel"]
                )
            else:
        ...
        for regime in REGIMES:
            for snapshot_idx in snapshots_by_regime[regime]:
                ...
                stages_with_snapshot = [
                    ...
                    if (
                        stage in results
                        and regime in results[stage]
                        and snapshot_idx in results[stage][regime]
                    )
                ]
    ...
    def _calculate_celltype_effectiveness(self, results: dict) -> dict:
        ...
        effectiveness = {
            "SST_relative": {
                "driven": {},
                "idle": {},
            },  # lambda_e_pv_only - lambda_full (SST vs PV-only)
            "PV_relative": {
                "driven": {},
                "idle": {},
            },  # lambda_e_sst_only - lambda_full (PV vs SST-only)
            "SST_absolute": {
                "driven": {},
                "idle": {},
            },  # lambda_e_only - lambda_e_sst_only (SST vs no inhibition)
            "PV_absolute": {
                "driven": {},
                "idle": {},
            },  # lambda_e_only - lambda_e_pv_only (PV vs no inhibition)
        }
        ...
        for condition in REGIMES:
            for stage in DEVELOPMENTAL_STAGES:
                ...
                for snapshot_idx in results[stage][condition]:
                    ...
                    sst_relative = [
                        e_pv - full
                        for full, e_pv in zip(lambda_full, lambda_e_pv_only, strict=False)
                    ]
                    pv_relative = [
                        e_sst - full
                        for full, e_sst in zip(lambda_full, lambda_e_sst_only, strict=False)
                    ]

                    # Calculate absolute effectiveness for each patch (how much each adds from baseline)
                    sst_absolute = [
                        e_only - e_sst
                        for e_only, e_sst in zip(lambda_e_only, lambda_e_sst_only, strict=False)
                    ]
                    pv_absolute = [
                        e_only - e_pv
                        for e_only, e_pv in zip(lambda_e_only, lambda_e_pv_only, strict=False)
                    ]
    ...
    def create_layer_specific_heatmaps(self, results: dict):
        ...
        for i, layer in enumerate(layer_order):
            for j, cell_type in enumerate(["SST", "PV"]):
                ...
                for k, stage in enumerate(x_labels):
                    ...
                    if cell_type == "SST":
                        if (
                            layer in effectiveness["SST_relative"]
                            and stage in effectiveness["SST_relative"][layer]
                        ):
                            effectiveness_matrix[row_idx, k] = effectiveness["SST_relative"][layer][
                                stage
                            ]["mean"]
                    else:  # PV
                        if (
                            layer in effectiveness["PV_relative"]
                            and stage in effectiveness["PV_relative"][layer]
                        ):
                            effectiveness_matrix[row_idx, k] = effectiveness["PV_relative"][layer][
                                stage
                            ]["mean"]
                    ...
                    if cell_type == "SST":
                        if (
                            layer in effectiveness["SST_absolute"]
                            and stage in effectiveness["SST_absolute"][layer]
                        ):
                            contribution_matrix[row_idx, k] = effectiveness["SST_absolute"][layer][
                                stage
                            ]["mean"]
                    else:  # PV
                        if (
                            layer in effectiveness["PV_absolute"]
                            and stage in effectiveness["PV_absolute"][layer]
                        ):
                            contribution_matrix[row_idx, k] = effectiveness["PV_absolute"][layer][
                                stage
                            ]["mean"]
        ...
        save_figure(fig, filepath_eff, dpi=DPI)
