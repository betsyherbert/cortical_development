...
from .config import ANALYSIS_PARAMS, DEVELOPMENTAL_STAGES, CELL_TYPES, LAYERS, PRESETS, DT, REGIMES
...
class StabilityAnalysis:
    ...
    def collect_snapshots(self, stage: str) -> Dict[str, List[Dict]]:
        ...
        print(f"  Running adaptive snapshot collection...")
            ...
            if not thresholds_computed and step > n_steps // 4:
                ...
                if 'early_buffer' in candidate_states:
                    for frame_idx, thal_sum, state in candidate_states['early_buffer']:
                        if thal_sum <= idle_threshold and len(snapshots['idle']) < self.params['n_snapshots']:
                            snapshots['idle'].append(state)
                        elif thal_sum >= driven_threshold and len(snapshots['driven']) < self.params['n_snapshots']:
                            snapshots['driven'].append(state)
                    del candidate_states['early_buffer']
                
                print(f"    Thresholds: idle ≤ {idle_threshold:.2f}, driven ≥ {driven_threshold:.2f}")
            
            if thresholds_computed:
                if (thalamic_sum <= idle_threshold and 
                    len(snapshots['idle']) < self.params['n_snapshots'] and
                    step % max(1, sample_interval // 2) == 0):
                    
                    state = self._capture_current_state(step, thalamic_sum, activities['thalamus'])
                    snapshots['idle'].append(state)
                    print(f"    Captured idle snapshot {len(snapshots['idle'])}/{self.params['n_snapshots']} at step {step}")
                
                elif (thalamic_sum >= driven_threshold and 
                      len(snapshots['driven']) < self.params['n_snapshots'] and
                      step % max(1, sample_interval // 2) == 0):
                    
                    state = self._capture_current_state(step, thalamic_sum, activities['thalamus'])
                    snapshots['driven'].append(state)
                    print(f"    Captured driven snapshot {len(snapshots['driven'])}/{self.params['n_snapshots']} at step {step}")
                
                if (len(snapshots['idle']) >= self.params['n_snapshots'] and 
                    len(snapshots['driven']) >= self.params['n_snapshots']):
                    print(f"    Early termination at step {step}/{n_steps}")
                    break
        
        print(f"  Collected {len(snapshots.get('idle', []))} idle and {len(snapshots.get('driven', []))} driven snapshots")
        return snapshots
    ...
    def compute_jacobian(self, patch_voltages: np.ndarray, patch_connections: np.ndarray, 
                        time_constants: np.ndarray, gains: np.ndarray) -> np.ndarray:
        """Compute voltage Jacobian matrix for a patch."""
        n_neurons = len(patch_voltages)
        
        heaviside = (patch_voltages > 0).astype(float)
        
        gains_heaviside = gains * heaviside
        jacobian = patch_connections * gains_heaviside[np.newaxis, :] / time_constants[:, np.newaxis]
        
        np.fill_diagonal(jacobian, -1.0 / time_constants)
        
        return jacobian
    
    def analyze_patch(self, snapshot: Dict, patch_coords: Tuple, analysis_type: str, 
                     condition: str = None):
        """
        ...
        if condition is not None:
            ...
            for layer_group in layers_to_analyze:
                if analysis_type == 'layer':
                    layer = layer_group
                    patch_data = self._extract_patch_data(snapshot, [layer], patch_coords, patch_size, condition)
                    lambda_max = self._compute_lambda_max(patch_data)
                    results[layer] = lambda_max
                else:
                    patch_data = self._extract_patch_data(snapshot, LAYERS, patch_coords, patch_size, condition)
                    lambda_max = self._compute_lambda_max(patch_data)
                    results = lambda_max
            return results
        else:
            ...
            for layer_group in layers_to_analyze:
                if analysis_type == 'layer':
                    ...
                    patch_data_full = self._extract_patch_data(snapshot, [layer], patch_coords, patch_size, 'full')
                    lambda_full = self._compute_lambda_max(patch_data_full)
                    
                    patch_data_e_only = self._extract_patch_data(snapshot, [layer], patch_coords, patch_size, 'e_only')
                    lambda_e_only = self._compute_lambda_max(patch_data_e_only)
                    
                    patch_data_e_pv_only = self._extract_patch_data(snapshot, [layer], patch_coords, patch_size, 'e_pv_only')
                    lambda_e_pv_only = self._compute_lambda_max(patch_data_e_pv_only)
                    
                    patch_data_e_sst_only = self._extract_patch_data(snapshot, [layer], patch_coords, patch_size, 'e_sst_only')
                    ...
                    patch_data_full = self._extract_patch_data(snapshot, LAYERS, patch_coords, patch_size, 'full')
                    lambda_full = self._compute_lambda_max(patch_data_full)
                    
                    patch_data_e_only = self._extract_patch_data(snapshot, LAYERS, patch_coords, patch_size, 'e_only')
                    lambda_e_only = self._compute_lambda_max(patch_data_e_only)
                    
                    patch_data_e_pv_only = self._extract_patch_data(snapshot, LAYERS, patch_coords, patch_size, 'e_pv_only')
                    lambda_e_pv_only = self._compute_lambda_max(patch_data_e_pv_only)
                    
                    patch_data_e_sst_only = self._extract_patch_data(snapshot, LAYERS, patch_coords, patch_size, 'e_sst_only')
                    lambda_e_sst_only = self._compute_lambda_max(patch_data_e_sst_only)
    ...
    def _extract_patch_data(self, snapshot: Dict, layers: List[str], patch_coords: Tuple, 
                           patch_size: int, condition: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        conn_strengths = tuple(sorted(
            (k, v['amplitude']) for k, v in self.simulation.circuit.connectivity.layer_params.items()
        ))
        ...
        if cache_key not in self._connection_cache:
            self._connection_cache[cache_key] = self._build_connection_matrix(layers, patch_size, condition)
        
        patch_connections = self._connection_cache[cache_key]
        
        return patch_voltages, patch_connections, time_constants, gains
    
    def _build_connection_matrix(self, layers: List[str], patch_size: int, condition: str) -> np.ndarray:
        ...
        def get_neuron_idx(layer_idx, dx, dy, cell_type_idx):
            return (layer_idx * patch_size * patch_size * len(CELL_TYPES) + 
        ...
        if condition in ['e_only', 'e_pv_only', 'e_sst_only']:
            for src_cell in CELL_TYPES:
                for tgt_cell in CELL_TYPES:
                    inhibitory_mask[(src_cell, tgt_cell)] = self._is_inhibitory_connection(src_cell, tgt_cell, condition)
                            ...
                            for tgt_dx in range(patch_size):
                                for tgt_dy in range(patch_size):
                                    for tgt_cell_idx, tgt_cell in enumerate(CELL_TYPES):
                                        tgt_idx = get_neuron_idx(tgt_layer_idx, tgt_dx, tgt_dy, tgt_cell_idx)
                                        
                                        conn_key = f'{layer}_{src_cell}_to_{tgt_layer}_{tgt_cell}'
                                        
                                        if conn_key in connectivity.layer_params:
                                            strength = connectivity.layer_params[conn_key]['amplitude']
                                            
                                            if condition in ['e_only', 'e_pv_only', 'e_sst_only'] and inhibitory_mask.get((src_cell, tgt_cell), False):
                                                strength = 0.0
    ...
    def _is_inhibitory_connection(self, source_cell: str, target_cell: str, condition: str = 'e_only') -> bool:
    ...
    def _analyze_snapshot(self, snapshot: Dict) -> Dict:
        ...
        for coord in layer_coords:
            ...
            for layer in LAYERS:
                ...
                results['layer_wise']['e_pv_only'][layer].append(layer_results[layer]['lambda_e_pv_only'])
                results['layer_wise']['e_sst_only'][layer].append(layer_results[layer]['lambda_e_sst_only'])
